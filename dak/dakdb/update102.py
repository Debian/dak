...
@copyright: 2013-2014, Ansgar Burchardt <ansgar@debian.org>
...
FROM
  (SELECT sa.suite AS suite, sa.source AS source, FALSE AS extra_source
   ...
   UNION
   SELECT ba.suite AS suite, b.source AS source, TRUE AS extra_source
...
COMMENT ON VIEW src_associations_full IS
  'all source packages for a suite, including sources for all binaries and those referenced by Built-Using'
""",
]
...
def do_update(self):
    ...
    try:
        ...
        c.execute("UPDATE config SET value = '102' WHERE name = 'db_revision'")
        self.db.commit()
